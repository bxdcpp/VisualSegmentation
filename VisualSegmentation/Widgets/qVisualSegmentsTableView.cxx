...
#include "qVisualSegmentsTableView.h"
#include "ui_qVisualSegmentsTableView.h"
...
//-----------------------------------------------------------------------------
class qVisualSegmentsTableViewPrivate: public Ui_qVisualSegmentsTableView
{
  Q_DECLARE_PUBLIC(qVisualSegmentsTableView);

protected:
  qVisualSegmentsTableView* const q_ptr;
public:
  qVisualSegmentsTableViewPrivate(qVisualSegmentsTableView& object);
  void init();
...
//-----------------------------------------------------------------------------
qVisualSegmentsTableViewPrivate::qVisualSegmentsTableViewPrivate(qVisualSegmentsTableView& object)
  : q_ptr(&object)
...
//-----------------------------------------------------------------------------
void qVisualSegmentsTableViewPrivate::init()
{
  ...
  this->ColumnLabels << "Visible" << QStringLiteral("颜色") << "Opacity" << QStringLiteral("分割名称");
  this->SegmentsTable->setHorizontalHeaderLabels(
    QStringList() << "" << QStringLiteral("颜色") << "Opacity" << QStringLiteral("分割名称"));
  ...
  this->SegmentsTable->horizontalHeader()->setSectionResizeMode(QHeaderView::Stretch);
  this->SegmentsTable->horizontalHeader()->setStretchLastSection(1);

  for (int i = 3; i < ColumnLabels.size() + 3; i++)
  {
	  this->SegmentsTable->setColumnWidth(i, 100);
  }
  //this->SegmentsTable->setColumnWidth(this->columnIndex(QStringLiteral("颜色")), 50);
  // Select rows
  ...
  //this->SegmentsTable->setItemDelegateForColumn(this->columnIndex(QStringLiteral("颜色")), itemDelegate);
  this->SegmentsTable->setItemDelegateForColumn(this->columnIndex(QStringLiteral("颜色")), new qSlicerTerminologyItemDelegate(this->SegmentsTable));
...
//-----------------------------------------------------------------------------
int qVisualSegmentsTableViewPrivate::columnIndex(QString label)
{
...
//-----------------------------------------------------------------------------
void qVisualSegmentsTableViewPrivate::setMessage(const QString& message)
{
...
//-----------------------------------------------------------------------------
QTableWidgetItem* qVisualSegmentsTableViewPrivate::findItemBySegmentID(QString segmentID)
{
  Q_Q(qVisualSegmentsTableView);
...
//------------------------------------------------------------------------------
QString qVisualSegmentsTableViewPrivate::getTerminologyUserDataForSegment(vtkSegment* segment)
{
  if (!segment)
    {
...
//-----------------------------------------------------------------------------
// qVisualSegmentsTableView methods

//-----------------------------------------------------------------------------
qVisualSegmentsTableView::qVisualSegmentsTableView(QWidget* _parent)
  : qMRMLWidget(_parent)
  , d_ptr(new qVisualSegmentsTableViewPrivate(*this))
{
  Q_D(qVisualSegmentsTableView);
  d->init();
  this->populateSegmentTable();
}

//-----------------------------------------------------------------------------
qVisualSegmentsTableView::~qVisualSegmentsTableView()
= default;

//-----------------------------------------------------------------------------
void qVisualSegmentsTableView::setSegmentationNode(vtkMRMLNode* node)
{
  Q_D(qVisualSegmentsTableView);
...
//---------------------------------------------------------------------------
void qVisualSegmentsTableView::setMRMLScene(vtkMRMLScene* newScene)
{
  Q_D(qVisualSegmentsTableView);
...
//-----------------------------------------------------------------------------
vtkMRMLNode* qVisualSegmentsTableView::segmentationNode()
{
  Q_D(qVisualSegmentsTableView);
...
//-----------------------------------------------------------------------------
QTableWidget* qVisualSegmentsTableView::tableWidget()
{
  Q_D(qVisualSegmentsTableView);
...
//-----------------------------------------------------------------------------
void qVisualSegmentsTableView::populateSegmentTable()
{
  Q_D(qVisualSegmentsTableView);
  ...
  foreach (QString segmentId, displayedSegmentIDs)
  {
    ...
    colorItem->setToolTip(qVisualSegmentsTableView::terminologyTooltipForSegment(segment));
    d->SegmentsTable->setItem(row, d->columnIndex(QStringLiteral("颜色")), colorItem);
...
//-----------------------------------------------------------------------------
void qVisualSegmentsTableView::updateWidgetFromMRML()
{
  Q_D(qVisualSegmentsTableView);
  ...
  foreach (QString segmentId, displayedSegmentIDs)
    ...
    // Terminology / color
    QTableWidgetItem* colorItem = d->SegmentsTable->item(row, d->columnIndex(QStringLiteral("颜色")));
    if (colorItem)
      {
...
//-----------------------------------------------------------------------------
void qVisualSegmentsTableView::onSegmentSelectionChanged(const QItemSelection &selected, const QItemSelection &deselected)
{
  Q_D(qVisualSegmentsTableView);
...
//-----------------------------------------------------------------------------
void qVisualSegmentsTableView::onSegmentTableItemChanged(QTableWidgetItem* changedItem)
{
  Q_D(qVisualSegmentsTableView);
  ...
  if (changedItem->column() == d->columnIndex(QStringLiteral("分割名称")))
    ...
    QTableWidgetItem* colorItem = changedItem->tableWidget()->item(changedItem->row(), d->columnIndex(QStringLiteral("颜色")));
    d->IsUpdatingWidgetFromMRML = true;
  ...
  // If visualization has been changed
    ...
    if (changedItem->column() == d->columnIndex(QStringLiteral("颜色")))
      {
      vtkSegment* segment = d->SegmentationNode->GetSegmentation()->GetSegment(segmentId.toLatin1().constData());
      if (!segment)
        {
      ...
      // Update tooltip
      changedItem->setToolTip(qVisualSegmentsTableView::terminologyTooltipForSegment(segment));
      }
    // Opacity changed
...
//-----------------------------------------------------------------------------
void qVisualSegmentsTableView::onVisibilityButtonClicked()
{
  Q_D(qVisualSegmentsTableView);
...
//-----------------------------------------------------------------------------
void qVisualSegmentsTableView::onVisibility3DActionToggled(bool visible)
{
...
//-----------------------------------------------------------------------------
void qVisualSegmentsTableView::onVisibility2DFillActionToggled(bool visible)
{
  QAction* senderAction = qobject_cast<QAction*>(sender());
  if (!senderAction)
    {
...
//-----------------------------------------------------------------------------
void qVisualSegmentsTableView::onVisibility2DOutlineActionToggled(bool visible)
{
  QAction* senderAction = qobject_cast<QAction*>(sender());
  if (!senderAction)
    {
...
//-----------------------------------------------------------------------------
void qVisualSegmentsTableView::setSegmentVisibility(QObject* senderObject, int visible, int visible3D, int visible2DFill, int visible2DOutline)
{
  Q_D(qVisualSegmentsTableView);
...
//-----------------------------------------------------------------------------
int qVisualSegmentsTableView::segmentCount() const
{
  Q_D(const qVisualSegmentsTableView);

  return d->SegmentsTable->rowCount();
}

//-----------------------------------------------------------------------------
QStringList qVisualSegmentsTableView::selectedSegmentIDs()
{
  Q_D(qVisualSegmentsTableView);
...
//-----------------------------------------------------------------------------
void qVisualSegmentsTableView::setSelectedSegmentIDs(QStringList segmentIDs)
{
  Q_D(qVisualSegmentsTableView);
...
//-----------------------------------------------------------------------------
void qVisualSegmentsTableView::clearSelection()
{
  Q_D(qVisualSegmentsTableView);
...
//------------------------------------------------------------------------------
bool qVisualSegmentsTableView::eventFilter(QObject* target, QEvent* event)
{
  Q_D(qVisualSegmentsTableView);
...
//------------------------------------------------------------------------------
void qVisualSegmentsTableView::endProcessing()
{
  this->populateSegmentTable();
}

//------------------------------------------------------------------------------
void qVisualSegmentsTableView::setSelectionMode(int mode)
{
  Q_D(qVisualSegmentsTableView);
...
//------------------------------------------------------------------------------
void qVisualSegmentsTableView::setHeaderVisible(bool visible)
{
  Q_D(qVisualSegmentsTableView);
...
//------------------------------------------------------------------------------
void qVisualSegmentsTableView::setVisibilityColumnVisible(bool visible)
{
  Q_D(qVisualSegmentsTableView);
...
//------------------------------------------------------------------------------
void qVisualSegmentsTableView::setColorColumnVisible(bool visible)
{
  Q_D(qVisualSegmentsTableView);
  d->SegmentsTable->setColumnHidden(d->columnIndex(QStringLiteral("颜色")), !visible);
}

//------------------------------------------------------------------------------
void qVisualSegmentsTableView::setOpacityColumnVisible(bool visible)
{
  Q_D(qVisualSegmentsTableView);
...
//------------------------------------------------------------------------------
void qVisualSegmentsTableView::setReadOnly(bool aReadOnly)
{
  Q_D(qVisualSegmentsTableView);
  if (aReadOnly)
    {
...
//------------------------------------------------------------------------------
int qVisualSegmentsTableView::selectionMode()
{
  Q_D(qVisualSegmentsTableView);
...
//------------------------------------------------------------------------------
bool qVisualSegmentsTableView::headerVisible()
{
  Q_D(qVisualSegmentsTableView);
...
//------------------------------------------------------------------------------
bool qVisualSegmentsTableView::visibilityColumnVisible()
{
  Q_D(qVisualSegmentsTableView);
...
//------------------------------------------------------------------------------
bool qVisualSegmentsTableView::colorColumnVisible()
{
  Q_D(qVisualSegmentsTableView);
  return !d->SegmentsTable->isColumnHidden(d->columnIndex(QStringLiteral("颜色")));
}

//------------------------------------------------------------------------------
bool qVisualSegmentsTableView::opacityColumnVisible()
{
  Q_D(qVisualSegmentsTableView);
...
//------------------------------------------------------------------------------
bool qVisualSegmentsTableView::readOnly()
{
  Q_D(qVisualSegmentsTableView);
...
//------------------------------------------------------------------------------
void qVisualSegmentsTableView::contextMenuEvent(QContextMenuEvent* event)
{
...
//------------------------------------------------------------------------------
void qVisualSegmentsTableView::showOnlySelectedSegments()
{
  QStringList selectedSegmentIDs = this->selectedSegmentIDs();
...
//------------------------------------------------------------------------------
void qVisualSegmentsTableView::jumpSlices()
{
  QStringList selectedSegmentIDs = this->selectedSegmentIDs();
  if (selectedSegmentIDs.size() == 0)
    {
...
//------------------------------------------------------------------------------
void qVisualSegmentsTableView::moveSelectedSegmentsUp()
{
  QStringList selectedSegmentIDs = this->selectedSegmentIDs();
  if (selectedSegmentIDs.size() == 0)
    {
...
//------------------------------------------------------------------------------
void qVisualSegmentsTableView::moveSelectedSegmentsDown()
{
  QStringList selectedSegmentIDs = this->selectedSegmentIDs();
  if (selectedSegmentIDs.size() == 0)
    {
...
// --------------------------------------------------------------------------
QString qVisualSegmentsTableView::terminologyTooltipForSegment(vtkSegment* segment)
{
  if (!segment)
    {
    qCritical() << Q_FUNC_INFO << ": Invalid segment given";
...
// --------------------------------------------------------------------------
void qVisualSegmentsTableView::setHideSegments(const QStringList& segmentIDs)
{
  Q_D(qVisualSegmentsTableView);
...
// --------------------------------------------------------------------------
QStringList qVisualSegmentsTableView::hideSegments()const
{
  Q_D(const qVisualSegmentsTableView);
...
// --------------------------------------------------------------------------
QStringList qVisualSegmentsTableView::displayedSegmentIDs()const
{
  Q_D(const qVisualSegmentsTableView);

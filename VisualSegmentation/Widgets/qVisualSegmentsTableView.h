...
#ifndef __qVisualSegmentsTableView_h
#define __qVisualSegmentsTableView_h
...
class vtkMRMLNode;
class vtkSegment;
class qVisualSegmentsTableViewPrivate;
class QStringList;
class QTableWidget;
...
class Q_SLICER_MODULE_VISUALSEGMENTATION_WIDGETS_EXPORT qVisualSegmentsTableView : public qMRMLWidget
{
  Q_OBJECT
  QVTK_OBJECT
...
public:
  ...
  /// Constructor
  explicit qVisualSegmentsTableView(QWidget* parent = nullptr);
  /// Destructor
  ~qVisualSegmentsTableView() override;
...
protected:
  QScopedPointer<qVisualSegmentsTableViewPrivate> d_ptr;

private:
  Q_DECLARE_PRIVATE(qVisualSegmentsTableView);
  Q_DISABLE_COPY(qVisualSegmentsTableView);
};
